...
type AnnotationFilter = "all" | "unaddressed" | "dismissed" | "fixed";
  ...
  return (
    <div className="no-print space-y-2.5 rounded-xl border border-slate-200 bg-slate-50 p-3 backdrop-blur-sm dark:border-white/10 dark:bg-white/5">
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {/* Search input */}
        <div className="relative w-full sm:ml-auto sm:w-auto sm:min-w-[180px]">
...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="inline-flex h-5 min-w-[20px] items-center justify-center rounded border border-slate-200 bg-white px-1 font-mono text-[10px] font-medium dark:border-white/10 dark:bg-white/5">
      {children}
    </kbd>
  );
}
  ...
  return (
    <div className="no-print flex flex-wrap items-center gap-x-4 gap-y-1 rounded-lg border border-slate-200 bg-slate-50/80 px-3 py-1.5 text-[11px] dark:border-white/10 dark:bg-white/[0.03]">
      <div className="flex items-center gap-1 text-slate-400 dark:text-white/30">
        <Kbd>j</Kbd><Kbd>k</Kbd>
        <span className="mx-0.5 text-slate-300 dark:text-white/10">or</span>
        <Kbd>{"\u2191"}</Kbd><Kbd>{"\u2193"}</Kbd>
        navigate
      </div>
      <div className="flex items-center gap-1 text-slate-400 dark:text-white/30">
      ...
      <span className="text-slate-200 dark:text-white/10" aria-hidden>|</span>
      {canAnnotate && (
        <div className={cn("flex items-center gap-1", hasSelection ? "text-slate-500 dark:text-white/40" : "text-slate-300 dark:text-white/15")}>
          <Kbd>f</Kbd> fixed
      ...
      <span className="text-[10px] text-slate-300 dark:text-white/15">click to select</span>
    </div>
  );
}
...
export function FeedbackList({ feedback, annotations, onAnnotate, onAddComment, onDeleteComment, onReplyComment, onResolveThread, commentSubmitting, onPageClick, conflicts, checkGroupOrder, reviewId, currentUserId, pageFilter, onAddFinding }: FeedbackListProps) {
  ...
  // Quality score (merged into the assessment banner)
  const { score, maxScore, scoreColor } = useQualityScore(feedback.findings);
  ...
  const filteredIndexed: IndexedFinding[] = useMemo(() => {
    ...
    for (let i = 0; i < feedback.findings.length; i++) {
      ...
      // Page filter (from heatmap click)
      if (pageFilter != null) {
        const referencesPage = f.locations.some((loc) => loc.page === pageFilter);
    ...
    return result;
  }, [feedback.findings, severityFilter, categoryFilter, annotationFilter, pageFilter, searchLower, annotations]);

  const { filtered, presentSeverities, gridColumns } = useMemo(() => {
    const groups = groupIndexedBySeverity(filteredIndexed);
    const sevs = SEVERITY_ORDER.filter((s) => groups[s] && groups[s]!.length > 0);
    const cols = sevs.map((sev) => (groups[sev] ?? []).map((item) => item.globalIndex));
    return { filtered: groups, presentSeverities: sevs, gridColumns: cols };
  }, [filteredIndexed]);

  // Annotation summary counts — only count entries that have a status set
  const totalFindings = feedback.findings.length;

  // 2D grid keyboard navigation + action shortcuts
  const handleNavAction = useCallback(
    (action: "fixed" | "dismissed" | "comment", globalIndex: number) => {
      if (action === "comment") {
        setTimeout(() => {
          const textarea = document.querySelector(`[data-finding-index="${globalIndex}"] textarea`) as HTMLTextAreaElement | null;
          textarea?.focus();
        }, 0);
      } else if (onAnnotate) {
        onAnnotate(globalIndex, action);
      }
    },
    [onAnnotate],
  );

  const { focusedGlobalIndex, setFocusedGlobalIndex } = useGridNavigation({
    columns: gridColumns,
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        ...
        className={cn(
          "rounded-xl border bg-gradient-to-r p-5 backdrop-blur-sm",
          config.gradient,
          config.border
        )}
      >
      ...
      {/* Shortcut bar — between filter and findings */}
              ...
              <div className="space-y-2">
                {items.map(({ finding, globalIndex }) => (
                  <div
                    ...
                    data-finding-index={globalIndex}
                    onClick={(e) => {
                      if ((e.target as HTMLElement).closest("button, a, input, textarea, select, [role=\"button\"]")) return;
                      setFocusedGlobalIndex(globalIndex);
                    }}
